...
STDsf <- readRDS("/Volumes/GoogleDrive/My Drive/Seed Transfer Project/Data/Merging/SeedTransferData_OSFclim_Aug2025.rds")
colnames(STDsf)
dim(STDsf)
...
##Export STDsf.csv with all vars and arranged by geodist
STDsf %>%
  st_drop_geometry() %>%
  arrange(desc(GeographicalDistance_km)) %>%
  write_csv("STDsf_sorted_by_geodist.csv")
